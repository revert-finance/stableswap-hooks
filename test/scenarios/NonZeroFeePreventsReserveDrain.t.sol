...
contract NonZeroFeePreventsReserveDrainTest is ExternalContractsDeployer {
    ...
    function test_exactInput_minNonZeroFee_cannotDrainReserve_poolStaysOperational() public {
        PoolKey memory poolKey = _getPoolKey();
        assertEq(poolKey.fee, MIN_LP_FEE, "Pool must use the minimum non-zero fee for this test");
        ...
        assertGt(hooks.reserves(1), 0, "non-zero fee must leave a reserve floor above zero");
    ...
    function _deployHooks() private {
        ...
        (, bytes32 salt) = factory.mineSalt(currencies, rateOracles, MIN_LP_FEE, AMP, code);

        hooks = StableSwapHooks(factory.deploy(currencies, rateOracles, MIN_LP_FEE, AMP, salt, code));
    }
