...
- **Ramp A down conservatively.** Swaps price against the live interpolated A, so a downward ramp lets a large trader round-trip across the drop and skim value from LPs (upward ramps are not affected). Mitigate, in order of strength: keep the LP fee high enough that a full round-trip across the ramp loses to fees (the actual gate); use a small down-multiplier (for example 3x-4x rather than 10x) to cap how much A can move; and ramp over weeks rather than the 1-day minimum to slow the drift. An adequately fee'd pool with active arbitrage is not exploitable in practice.
